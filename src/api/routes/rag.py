...
from api.rate_limit import (
    ...
    RAG_STREAM_RATE_LIMIT,
    VECTOR_SEARCH_RATE_LIMIT,
)
...
def rag_query(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"RAG query execution failed: {e!s}"
        )


@router.post("/query/stream")
@limiter.limit(RAG_STREAM_RATE_LIMIT)
    ...
    def event_generator():
        ...
        for event in rag.search_with_sources_stream(
            ...
            if event["type"] == "sources":
                retrieved_sources = event.get(
                    "sources",
                    []
                )

            elif event["type"] == "token":
                accumulated_summary += event.get(
                    "content",
                    ""
                )
...
@limiter.limit(VECTOR_SEARCH_RATE_LIMIT)
def vector_search(
    request: Request,
    req: QueryRequest
):
    ...
    try:
        ...
        raw_results = rag.vectorstore.query(
            req.query,
            top_k=req.top_k,
